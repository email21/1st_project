...
def get_last_recommended_product(conversation):
    """마지막으로 추천된 상품 정보 추출"""
    for message in reversed(conversation):
        if message["role"] == "assistant" and "📍 추천 상품:" in message["content"]:
            content = message["content"]
            # 추천 상품과 금융회사 정보 추출
            if "📍 추천 상품:" in content and "📍 금융회사:" in content:
                product_line = [line for line in content.split('\n') if '📍 추천 상품:' in line][0]
                company_line = [line for line in content.split('\n') if '📍 금융회사:' in line][0]
                return {
                    'product': product_line.split('📍 추천 상품:')[1].strip(),
                    'company': company_line.split('📍 금융회사:')[1].strip()
                }
    return None
        ...
        if prompt := st.chat_input("금융상품에 대해 질문해보세요!"):
            ...
            with st.spinner("추천 상품을 찾고 있습니다..."):
                try:
                    ...
                    # 마지막 추천 상품 정보를 시스템에 전달
                    last_product = get_last_recommended_product(st.session_state.conversation)
                    if last_product:
                        enhanced_prompt = f"이전 대화에서 추천한 상품: {last_product['product']} ({last_product['company']})\n사용자 질문: {prompt}"
                        result = rec_system.get_recommendation(enhanced_prompt)
                    else:
